...
std::unique_ptr<trustid::image::impl::DlibFaceVerificator> trustid::image::ServerImageProcessor::createVerificationModel(std::vector<FaceDetectionResultEntry> groundTruthResults)
{
    return std::make_unique<impl::DlibFaceVerificator>(groundTruthResults);
}

trustid::image::utils::ImageQualityResultEnum trustid::image::ServerImageProcessor::checkImageQuality(const cv::Mat image)
{
    return utils::checkImageQuality(image);
}

trustid::image::utils::ImageQualityResultEnum trustid::image::ServerImageProcessor::checkImageQuality(const FaceDetectionResultEntry detectionResultEntry)
{
