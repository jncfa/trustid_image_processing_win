...
trustid::image::ClientImageProcessor::ClientImageProcessor()
    : faceDetector(std::make_unique<impl::DlibFaceDetector>()) {}

trustid::image::ClientImageProcessor::ClientImageProcessor(
    const impl::DlibFaceVerificatorConfig config)
    : faceDetector(std::make_unique<impl::DlibFaceDetector>()),
      faceVerificator(
          std::make_unique<impl::DlibFaceVerificator>(std::move(config))) {}

void trustid::image::ClientImageProcessor::loadFaceVerificationData(
    const impl::DlibFaceVerificatorConfig config) {
  faceVerificator = std::make_unique<impl::DlibFaceVerificator>(config);
}

bool trustid::image::ClientImageProcessor::canVerifyFaces() {
  return faceVerificator != nullptr;
}

trustid::image::FaceDetectionResult
trustid::image::ClientImageProcessor::detectFaces(const cv::Mat image) {
  return faceDetector->detectFaces(image);
}

trustid::image::FaceVerificationResult
trustid::image::ClientImageProcessor::verifyUser(
    const FaceDetectionResultEntry detectionResultEntry) {
  return faceVerificator->verifyUser(detectionResultEntry);
}

trustid::image::HeadPoseEstimationResult
trustid::image::ClientImageProcessor::estimateHeadPose(
    const FaceDetectionResultEntry detectionResultEntry) {
...
trustid::image::ClientImageProcessor::checkImageQuality(const cv::Mat image) {
  return utils::checkImageQuality(image);
}

trustid::image::utils::ImageQualityResultEnum
trustid::image::ClientImageProcessor::checkImageQuality(
    const FaceDetectionResultEntry detectionResultEntry) {
