...
// TODO: Disabling Protobuf for now, as it causes issues with the build, and we don't need it for now
// #include "face_verificator_config.pb.h"
...
using ResNet34 = dlib::loss_metric<dlib::fc_no_bias<128, dlib::avg_pool_everything<
                                                             alevel0<
                                                                 alevel1<
                                                                     alevel2<
                                                                         alevel3<
    ...
    namespace image
    {
        namespace impl
        {
            ...
            struct DlibFaceVerificatorConfig
            {
                ...
                DlibFaceVerificatorConfig(const std::vector<dlib::matrix<float, 0, 1>> groundTruthVecs, const ResNet34 net, const float distanceThreshold = 0.6, const float votingThreshold = 0.5) : groundTruthVecs(groundTruthVecs), net(net), distanceThreshold(distanceThreshold), votingThreshold(votingThreshold) {}
                ...
                DLIB_DEFINE_DEFAULT_SERIALIZATION(DlibFaceVerificatorConfig, groundTruthVecs, net, distanceThreshold, votingThreshold);
            };

            class DlibFaceVerificator : public IFaceVerificator
            {
            public:
                DlibFaceVerificator(const std::vector<FaceDetectionResultEntry> groundTruthChips, const float distanceThreshold = 0.6, const float votingThreshold = 0.5);
                DlibFaceVerificator(const DlibFaceVerificatorConfig config);

                DlibFaceVerificatorConfig getConfig();

            private:
                virtual FaceVerificationResult _verifyUser(const FaceDetectionResultEntry detectionResultEntry) override;
